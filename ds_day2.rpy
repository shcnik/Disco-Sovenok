...
label ds_day2_morning:
    ...
    window hide
    menu:
        "Умыться" (skill='pain_threshold', level=lvl_heroic):
            if ds_last_skillcheck.result():
                window show
                play sound ds_sfx_fys
                pat "{result}Однако, тебе удаётся вытерпеть холод и как следует почистить зубы."
            else:
                window show
                play sound ds_sfx_fys
                pat "{result}Всё-таки холод воды невыносим. Тебе больно от неё, физически больно."
    ...
    window hide
    menu:
        "Поздороваться" (skill='rhetoric', level=lvl_easy):
            if ds_last_skillcheck.result():
                window show
                play sound ds_sfx_int
                rhe "{result}Тут ничего сложного, «привет» достаточно."
                me "Привет!"
            else:
                window show
                play sound ds_sfx_int
                rhe "{result}Приветствие тебе удаётся выбрать не сразу."
                me "Охай… То есть, бобр… Доброе утро! Вот…"
            
        "Начать приставать" (skill='instinct', level=lvl_medium):
            if ds_last_skillcheck.result():
                window show
                ins "{result}Её фигура настолько привлекательная, а форма так облегает... руки сами тянутся."
                show sl serious sport at center with dspr
                sl "Куда руки тянешь?"
                $ ds_lp['sl'] -= 1
            else:
                window show
                ins "{result}Но у тебя нет никакой реакции на фигуру. Стимула недостаточно!"
            
        "Предложить заниматься вместе" (skill='volition', level=lvl_up_medium):
            if ds_last_skillcheck.result():
                window show
                vol "{result}Пригласить её заниматься вместе спортом - что может быть проще?"
                ...
                window show
                vol "{result}Но ты не решаешься пригласить её заниматься вместе... чего-то боишься."
                "Ты молчишь."
    ...
    menu:
        "Забежать":
            ...
            menu:
                ...
                "Приставать к ней" (skill='instinct', level=lvl_easy):
                    if ds_last_skillcheck.result():
                        window show
                        play sound ds_sfx_fys
                        ins "{result}У тебя в штанах от наблюдаемой сцены настоящий праздник."
                        ins "Ты подходишь к ней и хватаешь её за спину."
                        scene cg ds_day2_mt_undress2
                    else:
                        window show
                        play sound ds_sfx_fys
                        ins "{result}Ты настолько перенервничал от неудобства ситуации, что ничего сделать не можешь."
                        scene cg ds_day2_mt_undress2
                        "Наконец вожатая замечает твоё присутствие..."
                    
                "Тихо выйти из домика" (skill='savoir_faire', level=lvl_medium):
                    if ds_last_skillcheck.result():
                        window show
                        play sound ds_sfx_mot
                        ...
                        window show
                        play sound ds_sfx_mot
                        svf "{result}Под тобой предательски скрипит ступенька, и Ольга Дмитриевна замечает тебя!"
                        scene cg ds_day2_mt_undress2
...
label ds_day2_mt_out:
    ...
    window hide
    menu:
        "Притвориться, что слушал" (skill='drama', level=lvl_medium):
            if ds_last_skillcheck.result():
                dra "{result}Вы ясно даёте понять своим видом, что всё поняли."
                me "Да, понял."
                $ ds_karma += 5
                show mt smile panama pioneer at center with dspr
                mt "Вот и отлично!"
                ...
                dra "{result}Но твоё выражение лица всё выдаёт!"
                show mt angry panama pioneer at center   with dspr
                mt "Ты не слушаешь!"
                me "Простите…"
                show mt normal panama pioneer at center   with dspr
    ...
    if ds_beat_dv:
        ...
        with dissolve
        mt "А теперь к плохим новостям!"
        mt "Вчера вечером новенький Семён встретил своих товарищей по лагерю не особо по-пионерски."
        mt "А именно - побил всем известную Алису Двачевскую!"
        show us upset pioneer far at fleft
        show mz angry pioneer far at left
        show mi shocked pioneer far at cleft
        ...
        with dspr
        voices "Чего? Как так?"
        mt "Я очень надеюсь, что Семён прямо сейчас извинится и пообещает больше так не делать!"
        window hide
        menu:
            "Извиниться":
                window show
                me "Извини, Алиса... я так не хотел."
                $ ds_karma += 5
                show dv angry pioneer far at right with dspr
                dv "Ага, не хотел. Ты думаешь, что побил девушку и можешь просто сказать «извини»?!"
                mt "Алиса!.."
                mt "А впрочем, я её прекрасно понимаю. Ты повёл себя не как достойный гражданин Советского Союза и в принципе человек!"
                mt "Всё, иди в строй!"
            "Не извиняться" (skill='composure', level=lvl_godly):
                if ds_last_skillcheck.result():
                    window show
                    com "{result}Держись ровно и не кайся!"
                    ...
                    $ ds_karma -= 10
                    mt "Ты в любом случае должен будешь извиниться! Иди в строй!"
                else:
                    window show
                    com "{result}Однако, у тебя не получится выдержать всего этого общественного осуждения."
                    com "Да и поступил ты вообще говоря нехорошо."
                    me "Я... не знаю, что сказать..."
                    show mt rage pioneer at fleft with dspr
                    mt "Да что-нибудь уж скажи! Извини, прости!.."
                    me "Ладно... мне очень жаль..."
                    mt "Иди уже в строй, потом разберёмся!"
    elif ds_beat_sl:
        show mt angry pioneer at fleft
        with dissolve
        mt "А теперь к плохим новостям!"
        mt "Вчера вечером новенький Семён мало того, что проник в столовую вместе со всем известной Алисой Двачевской - он ещё и побил застукавшую их Славю!"
        show us upset pioneer far at fleft
        show mz angry pioneer far at left
        show mi shocked pioneer far at cleft
        ...
        menu:
            "Извиниться":
                window show
                me "Извини, Славя... я так не хотел."
                $ ds_karma += 5
                show sl normal pioneer far at right with dspr
                dv "Ладно, на этот раз я тебя прощу!"
                mt "Всё, иди в строй!"
            "Не извиняться" (skill='composure', level=lvl_godly):
                if ds_last_skillcheck.result():
                    window show
        ...
        with dissolve
        mt "Давайте поприветствуем нашего нового товарища Семёна!"
        voices "Ура, приветствуем!"
        if ds_skill_list['volition'].check(lvl_easy, passive=True).result():
            vol "{result}Поприветствуй их тоже."
            me "Приветствую! Надеюсь, лето, проведённое здесь, будет самым лучшим!"
        else:
            vol "{result}Ты не привык к такому вниманию и напуган."
            "Ты молчишь."
            mt "Cемён, скажи что-нибудь!"
            me "Э... привет..."
        mt "Ладно, возвращайся в строй!"
    scene cg ds_day2_lineup
    with dissolve
    ...
    window hide
    menu:
        "Пойти с Алисой" (skill='volition', level=lvl_challenging):
            window show
            ...
            if ds_last_skillcheck.result():
                play sound ds_sfx_psy
                vol "{result}Всё получится! Иди к ней!"
                ...
                if ds_skill_list['authority'].check(lvl_up_medium, passive=True).result():
                    play sound ds_sfx_psy
                    aut "{result}А сейчас ты её убедишь пойти с тобой."
                    ...
                    play sound ds_sfx_psy
                    aut "{result}У тебя не хватает решимости потребовать у Алисы пойти с тобой."
                ...
                vol "{result}Но нет, тебе не хватает решимости подойти к ней и заговорить."
                th "Ладно, наверное, лучше будет пойти одному..."
                jump ds_day2_pass_alone_main
        "Пойти с Леной" (skill='volition', level=lvl_medium):
            th "А стоит ли подходить к Лене? Напугаю её еще..."
            if ds_last_skillcheck.result():
                play sound ds_sfx_psy
                vol "{result}Давай, подойди к ней!"
                ...
                play sound ds_sfx_psy
                vol "{result}Нет, ты слишком боишься подходить к девушкам."
...
label ds_day2_pass_alone_music:
    ...
    menu:
        "Сдержать порывы":
            window show
            me "Простите…"
        "Начать приставать" (skill='instinct', level=lvl_trivial):
            if ds_last_skillcheck.result():
                window show
                ins "{result}Внизу у тебя встаёт твой ствол..."
                ins "Ты подходишь к ней и прижимаешься ей к юбке..."
                play sound ds_sfx_mot
                res "Но она тебя замечает!"
            else:
                window show
                ins "{result}Ты хочешь к ней пристать, но что-то идёт не так."
                ins "Извини."
    ...
    if ds_skill_list['encyclopedia'].check(lvl_medium, passive=True).result():
        enc "{result}Точно, Мику существует и в твоём мире..."
        ...
        enc "{result}Нет, показалось. Ты никогда не слышал имени Мику."
...
label ds_day2_pass_alone_clubs:
    ...
    if ds_skill_list['encyclopedia'].check(lvl_easy, passive=True).result():
        enc "{result}Точно, он же вылитый Шурик из комедий Гайдая!"
    else:
        enc "{result}Хотя нет, показалось..."
        ...
        "Не будить":
            ...
            menu:
                "Почитать":
                    ...
                    $ ds_read_book = True
                    play sound ds_sfx_int
                    lgc "Покуда ты в библиотеке, попробуй найти что-нибудь по местной истории. Судя по Генде, она тут немного отличается от нашей."
                    "На одной из ближайших полок оказывается «История СССР. XX век»."
                    "Ты открываешь на параграфах, идущих сразу после правления Сталина."
                    
                    book "После смерти вождя народов встал вопрос о назначении преемника. 5 марта 1953 года председателем Совета Министров СССР стал один из видных соратников великого Сталина, Георгий Максимилианович Маленков."
                    book "В сложившейся после Великой Отечественной войны системе фактически обладатель именно этой должности считался преемником. Это обусловило избрание Маленкова на пост генерального секретаря ЦК КПСС 8 февраля 1955 года."
                    book "С самого начала Маленков начал ослаблять наиболее чрезмерные ограничения: запрет на иностранную прессу, пересечение границ. Также прекратила преследоваться критика действий партии и правительства, остававшаяся в рамках социалистической идеологии."
                    book "в 1956 году состоялся XX съезд КПСС. На нём были подвергнуты осуждению позиции ревизионистов во главе с Н.С. Хрущёвым. Сам Хрущёв был отправлен в отставку. По итогам съезда было признано, что несмотря на некоторые ошибки, политика Сталина была в целом правильной."
                    book "Пост главы МВД сохранил Л.П. Берия. При нём была произведена частичная амнистия репрессированных из-за перегибов на местах. При этом, вопреки утверждениям империалистического Запада и антисоветчиков, было показано, что большинство репрессированных были осуждены не за «политические», а за общеуголовные статьи."
                    book "На внешнеполитическом фронте удалось укрепить позиции социалистического лагеря. Венцом триумфа стало присоединение к СССР двух новых республик - Финляндии (1956) и Монголии (1954). Также следует отметить возвращение в социалистический лагерь отколовшейся ранее Югославии."
                    
                    book "В 1959 году Анатолий Иванович Китов предложил создать общегосударственную автоматизированную систему управления народным хозяйством. Данная идея с подачи Генерального секретаря получила одобрение в партии, был создан Госкомупр - единый орган, координировавший разработку АСУ."
                    book "С этого момента начинается история проекта ОГАС - Общегосударственной автоматизированной системы учёта и обработки информации. Однако, в полной мере её смогли развернуть при следующем Генеральном секретаре - Икари Генде (1967-1985)."
                    book "Икари Генда родился в Японии в 1925 году. В ходе репрессий, осуществляемых японскими милитаристами в 1930-х годах, его семья эмигрировала в СССР. Генда принял коммунистические идеи, переданные ему родителями, и, получив гражданство СССР, тут же вступил в ВКП(б)."
                    book "Будучи избранным на пост Генерального секретаря ЦК КПСС, И. Генда провозгласил главным приоритетом продвижение науки и технологий, в частности развитие ЭВМ. Кульминацией данной политики стала разработка и внедрение системы ОГАС, что позволило начать частичную отмену товарно-денежных отношений и начать тем самым переход к коммунизму."
                    book "Также была провозглашена политика партийного плюрализма: допускалось существование нескольких фракций со своими видениями дальнейшего пути развития СССР. Это позволило принимать наилучшие решения на основе дискуссии. Вкупе с этим был реализован проект ещё Сталина по альтернативным выборам в Верховный Совет из нескольких кандидатов."
                    book "В это время усилия советской дипломатии позволили объединить два разобщённых империалистами народа - немцев и корейцев. ГДР и ФРГ объединились в единую Германию на условиях нейтралитета. То же произошло и с КНДР и Южной Кореей, образовавших Конфедерацию Корея. Оба государства стали членами СЭВ."
                    book "Также И.Генда обратил внимание и на свою родину Японию. В 1977 году группа американских солдат совершила групповое изнасилование девочки. Данный инцидент был бы замят, если бы не вмешательство КГБ, предъявившего правду японскому обществу. Под давлением народных масс японское правительство разорвало все договоры с США и приняло политику нейтралитета."
                    book "Фактически, это позволило начать сближение Японии и СССР в экономическом и культурном аспектах. Также за счёт разрыва отношений с США удалось достичь примирения Японии с одной стороны и Кореи и Китая с другой. И по сей день Япония остаётся одним из крупнейших партнёров СССР за пределами соцлагеря."

                    book "В 1985 году И. Генду сменил Григорий Васильевич Романов. Новый Генеральный секретарь объявил о приверженности линии его предшественников по соединению жёсткой линии и реформ по дальнейшему внедрению ОГАС."
                    book "Начало правления Романова отметилось двумя крупными космическими успехами СССР - запуском космической станции «Мир» в 1985 году и созданием первой лунной базы в 1986 году. Помимо этого, компьютеризация Советского Союза достигла таких высот, что стало возможным создание сети, объединяющей все ЭВМ и информацию на них - т.н. «интернета»."
                    book "Также, в 1986 году была совершена бомбардировка лагерей моджахедов в Пакистане, что позволило вскоре завершить Афганскую войну победой правительственных сил, несмотря на осуждение западных стран. Афганистан стал одним из ближайших союзников СССР и по мере налаживания порядка - потенциальной новой союзной республикой."
                    book "Одно можно сказать наверняка - с использованием последних достижений науки, особенно кибернетики, наступление светлой эры коммунизма уже не за горами. А усиливающиеся противоречия капитализма на фоне успехов социализма позволяют верить, что вскоре нас ждёт всемирный триумф идей марксизма-ленинизма."

                    play sound ds_sfx_int
                    lgc "Итак - в данном мире история СССР пошла по другому пути ещё с 1953 года. И, как следствие, тут Союз не то, что не собирается распадаться - вполне имеет все шансы победить в Холодной войне."
...
label ds_day2_pass_alone_lunch:
    ...
    menu:
        "Отреагировать":
            ...
            "Ты ухмыляешься."
        "Игнорировать" (skill='composure', level=lvl_challenging):
            if ds_last_skillcheck.result():
                window show
                play sound ds_sfx_mot
                com "{result}Ты делаешь вид, будто ничего не происходит и углубляешься в обед."
                ...
                window show
                play sound ds_sfx_mot
                com "{result}Ты бы и хотел сделать вид, что её нет, но она тебя слишком уж раздражает."
                me "Я тебя когда-нибудь удушу!"
                show us laugh2 pioneer at center   with dspr
                us "Не догонишь!"
                "Она показывает тебе язык!"
    ...
    window hide
    menu:
        "Подшутить" (skill='drama', level=lvl_easy):
            if ds_last_skillcheck.result():
                window show
                dra "{result}Напугайте её вожатой, мессир."
                ...
                dra "{result}Но у вас совсем нет идей, как бы подшутить, мессир."
...
label ds_day2_pass_dv_clubs:
    ...
    window hide
    menu:
        "Остановить Алису" (skill='savoir_faire', level=lvl_legendary):
            window show
            ...
            if ds_last_skillcheck.result():
                window show
                svf "{result}Ты ловким движением останавливаешь руку Алисы."
                ...
                svf "{result}Однако ты влезаешь неудачно, и кулак Алисы, отскочив от глаза Электроника, прилетает тебе в голову."
                $ ds_health -= 1
    ...
    window hide
    menu:
        "Обратить внимание на Алису" (skill='empathy', level=lvl_up_medium):
            if ds_last_skillcheck.result():
                
                play sound ds_sfx_psy
                emp "{result}Ты замечаешь, что с Алисой что-то не так."
                me "Алис?.."
                show dv sad pioneer2 at center with dspr
                ...
                play sound ds_sfx_psy
                emp "{result}Вроде всё нормально."
        "Пойти дальше":
            pass
...
label ds_day2_pass_dv_medic:
    ...
    window hide
    menu:
        "Требовать продолжения" (skill='instinct', level=lvl_easy):
            if ds_last_skillcheck.result():
                window show
                ins "{result}Она {i}хочет{/i} тебя. И это взаимно. Требуй продолжения!"
                ...
                window show
                ins "{result}Но ты не хочешь продолжения! Тебя она не привлекает."
...
label ds_day2_pass_dv_library:
    ...
    if ds_skill_list['reaction_speed'].check(lvl_medium, passive=True).result():
        play sound ds_sfx_mot
        res "{result}На миг ты замечаешь, что появление Лены вызвало неприязненную реакцию Алисы."
...
label ds_day2_pass_dv_lunch:
    ...
    menu:
        ...
        "Заставить Алису стоять вместе":
            ...
            me "Я нашёл столик!"
            scene bg ds_int_dininghall_table2_day
            show dv normal pioneer2 at center, ds_sit_down
            with dissolve
    ...
    menu:
        "Заговорить с Алисой":
            ...
            window hide
            menu:
                "Начать разговор" (skill='rhetoric', level=lvl_challenging):
                    if ds_last_skillcheck.result():
                        window show
                        rhe "{result}Набор вопросов стандартный - откуда она, чем увлекается."
                        ...
                        window show
                        rhe "{result}Тебе не приходят в голову никакие темы."
                        
                        "Вы молча доедаете обед и идёте подписывать обходной."
                "Проанализировать её поведение" (skill='empathy', level=lvl_legendary):
                    if ds_last_skillcheck.result():
                        window show
                        play sound ds_sfx_psy
                        emp "{result}Она из-за чего-то сильно переживает."
                        ...
                        window show
                        play sound ds_sfx_psy
                        emp "{result}Тебе решительно непонятно её поведение."
...
label ds_day2_pass_sl_music:
    ...
    if ds_sl_keys:
        ...
        menu:
            ...
            "Отцепить ключик себе" (skill='interfacing', level=lvl_godly):
                if ds_last_skillcheck.result():
                    window show
                    ...
                    menu:
                        "Сказать про ключи":
                            ...
                            $ ds_sl_keys = False
                            emp "Она тебе благодарна."
                        "Cоврать" (skill='drama', level=lvl_formidable):
                            if ds_last_skillcheck.result():
                                window show
                                play sound ds_sfx_int
    ...
    menu:
        "Сдержать порывы":
            window show
            me "Простите…"
        "Начать приставать" (skill='instinct', level=lvl_easy):
            if ds_last_skillcheck.result():
                window show
                ins "{result}Внизу у тебя встаёт твой ствол..."
                ins "Ты подходишь к ней и прижимаешься ей к юбке..."
                ...
                window show
                ins "{result}Ты бы и начал к ней приставать, но тут Славя..."
                "Твои неуклюжие движения привлекают внимание девочки."
...
label ds_day2_pass_un_music:
    ...
    if (ds_passed_places == 3) and not ds_had_lunch:
        ...
        menu:
            "Отпустить":
                ...
                hide un with dissolve
                jump ds_day2_pass_alone_music
            "Остановить" (skill='suggestion', level=lvl_up_medium):
                if ds_last_skillcheck.result():
                    window show
                    sug "{result}Вы быстро обойдёте всё остальное и сможете поесть."
                    me "Нет, подожди... мы быстренько! И сходим пообедаем."
                    un "Ладно..."
                    $ ds_lp['un'] -= 1
                else:
                    window show
                    sug "{result}Ты не знаешь, что сказать - она же определённо хочет есть."
                    me "Ну, останься..."
    ...
    menu:
        ...
        "Начать приставать" (skill='instinct', level=lvl_easy):
            if ds_last_skillcheck.result():
                ...
                window show
                ins "{result}Ты бы и начал к ней приставать, но тут Лена..."
                "Твои неуклюжие движения привлекают внимание девочки."
    ...
    menu:
        "Спросить про Алису":
            ...
            if ds_skill_list['drama'].check(lvl_challenging, passive=True).result():
                play sound ds_sfx_int
                dra "{result}Нет, она от вас что-то скрывает, мессир."
...
label ds_day2_pass_un_medic:
    ...
    window hide
    menu:
        "Настоять пойти вместе" (skill='suggestion', level=lvl_medium):
            if ds_last_skillcheck.result():
                window show
                sug "{result}Тебе не справиться без неё. Акцентируй внимание на этом."
                ...
                window show
                sug "{result}Она сказала нет - значит, нет."
                me "Ладно, я пойду."
                "Ты входишь."
                window hide
...
label ds_day2_pass_un_lunch:
    ...
    window hide
    menu:
        "Заговорить с Леной" (skill='rhetoric', level=lvl_up_medium):
            if ds_last_skillcheck.result():
                window show
                play sound ds_sfx_int
                rhe "{result}Начни расспрашивать про её интересы. Про то, как она попала в лагерь. Вообще про неё!"
                        ...
                        menu:
                            "Рассказать как есть":
                                ...
                                if ds_skill_list['reaction_speed'].check(lvl_easy, passive=True).result():
                                    play sound ds_sfx_mot
                                    res "{result}Нет тут Санкт-Петербурга, только Ленинград!"
                                    ...
                                    un "Ну ладно..."
                            "Придумать" (skill='conceptualization', level=lvl_formidable):
                                if ds_last_skillcheck.result():
                                    window show
                                    con "{result}Заграница. Ты из-за границы. Из ГДР, например."
                                    ...
                                    window show
                                    con "{result}Ничего интереснее Москвы тебе не приходит в голову."
                ...
                window show
                play sound ds_sfx_int
                rhe "{result}Тебе не удаётся придумать темы для разговора с Леной."
                play sound ds_sfx_psy
...
label ds_day2_after_pass:
    ...
    window hide
    menu:
        "Возмутиться праздностью" (skill='authority', level=lvl_medium):
            if ds_last_skillcheck.result():
                window show
                play sound ds_sfx_psy
                aut "{result}Выскажи всё, что ты думаешь о ней!"
                ...
                window show
                play sound ds_sfx_psy
                aut "{result}Нет, лучше будь осторожным."
                me "Вот…"
    ...
    if (ds_member['music'] and ds_member['cyber']) or (ds_member['music'] and ds_member['sport']) or (ds_member['cyber'] and ds_member['sport']):
        ...
        menu:
            ...
            "Остаться в спортклубе" if ds_member['sport']:
                ...
                show mt normal panama pioneer at center with dspr
                mt "Хорошо, я скажу об этом главам остальных клубов."
            "Настоять на своём" (skill='suggestion', level=lvl_medium):
                if ds_last_skillcheck.result():
                    window show
                    sug "{result}Это же ей лучше, если ты сможешь везде ходить. Правилами это не запрещено, так что вперёд. Используй лозунги."
                    ...
                    window show
                    sug "{result}Но, вообще она права - ты и правда всюду не поспеешь."
    ...
    menu:
        "Cесть с Алисой" if ds_eldv_side_taken != -1:
            ...
            show dv normal pioneer at center, ds_seated
            with dissolve
            ...
            window hide
            menu:
                "Попытаться выяснить" (skill='empathy', level=lvl_challenging, modifiers=[('ds_support_dv > 0', 2, 'Поддерживал Алису'), ('ds_eldv_side_taken == 1', 2, 'Взял сторону Алисы')]):
                    if ds_last_skillcheck.result():
                        play sound ds_sfx_psy
                        emp "{result}Алису по какой-то причине сильно задевает прозвище «ДваЧе». Попробуй выяснить, с чем это связано."
                        ...
                        play sound ds_sfx_psy
                        emp "{result}Видимо, Алисе не понравилась сложившаяся ситуация... но ничего больше ты понять не можешь."
        ...
        "Cесть с Ульяной" if ds_eldv_side_taken != -1:
            window show
            $ ds_lp['us'] += 1
            show us smile pioneer at center, ds_seated
            with dissolve
            play sound ds_sfx_fys
            hfl "Ну, готовься..."
            us "Привет!"
            ...
            if ds_lp['us'] >= 3:
                ...
                menu:
                    ...
                    "Отказаться":
                        ...
                        $ ds_lp['us'] -= 1
                        $ ds_morale.damage()
                    "Убедить не сбегать" (skill='rhetoric', level=lvl_formidable):
                        if ds_last_skillcheck.result():
                            window show
                            rhe "{result}Собственно, то, что найдут и накажут - это правда. А найдут обязательно."
                ...
                if ds_lp['dv'] >= 15:
                    ...
                    window hide
                    menu:
                        "Надавить" (skill='authority', level=lvl_easy):
                            if ds_last_skillcheck.result():
                                play sound ds_sfx_psy
                                aut "{result}Заставь её сказать! Выбей из неё... эм, фигурально. Она же ребёнок!"
                                ...
                                play sound ds_sfx_psy
                                aut "{result}Ты пытаешься пригрозить ей, но в твоём исполнении слишком уж потешно это выгялдит."
                                me "Скажи."
                                show us laugh pioneer at center with dspr
        ...
        "Сесть с Мику":
            ...
            show mi normal pioneer at center, ds_seated
            with dissolve
...
label ds_day2_cards_alone:
    ...
    if ds_skill_list['shivers'].check(lvl_medium, passive=True).result():
        play sound ds_sfx_fys
        shi "{result}Возможно, карты не там. Какое-то такое ощущение у тебя есть..."
        window hide
        menu:
            "Переспросить":
                window show
            ...
            "Использовать ключи":
                ...
                menu:
                    ...
                    "Продолжить изучать":
                        ...
                        play sound ds_sfx_mot
                        per_eye "Впрочем, никаких секретов тут не оказывается - если не считать таковыми твои документы."
                        per_eye "Если конкретно - тут лежат «твой» паспорт СССР и досье."
                        ...
                        window hide
                        menu:
                            "Изобразить невинного" (skill='drama', level=lvl_godly):
                                if ds_last_skillcheck.result():
                                    window show
                                    dra "{result}Покажите, что вы, напротив, закрывали ящик, не открывали."
                                    ...
                                    window show
                                    dra "{result}У вас, увы, не получается показать, что вы не при делах."
                            ...
                            "Нагло продолжить читать":
                                ...
                                $ ds_semtype += 1
                                "CССР - ПАСПОРТ."
                                "На первой странице - герб СССР (кстати, с 19 лентами), ниже - «ПАСПОРТ ГРАЖДАНИНА СОЮЗА СОВЕТСКИХ СОЦИАЛИСТИЧЕСКИХ РЕСПУБЛИК»"
                                "Затем машинописным шрифтом написаны твои фамилия, имя, отчество - ПЁРСУНОВ СЕМЁН КАМИНОВИЧ"
                                "На следующем развороте год рождения - 21 декабря 1971 года; место рождения - город Ленинград, РСФСР; национальность - русский."
                                play sound ds_sfx_int
                                enc "Всё совпадает. Кроме года - но возраст также совпадает."
                                "Информацию о выдаче ты пропускаешь и переходишь к следующему развороту."
                                "Твоё фото. Естественно, соответствующее твоему статусу пионера в плане возраста."
                                $ ds_know_face = True
                                "А теперь самое интересное - место жительства. Город Ленинград, 3-я улица Строителей, дом 25, квартира 12."
                                "Дальше идёт куча пустых страниц, так что ты решаешь перейти к своему личному делу."
                                th "Итак, кто же я в этом мире?"
                                "Первая же страница тебя ошарашивает: тут заявлено, что твой отец - посол СССР в Японии."
                                $ ds_know_parents = True
                                play sound ds_sfx_int
                                dra "Да вы, оказывается, мажор, мессир."
                                dra "Теперь мы наконец-то знаем, кого отыгрывать!"
                                th "Так, что дальше? Моя мама сейчас министром окажется, что ли?"
                                "Но нет - твоя мать не занимает никаких должностей."
                                "Далее досье утверждает, что учишься ты отлично, нацелен на поступление в Московский физико-технический институт."
                                "Однако, ты отличаешься рассеянностью. Вполне можешь и в зимней одежде приехать летом."
                                play sound ds_sfx_int
                                lgc "Так вот почему никто не удивился тому, что ты в одежде не по погоде!"
                                "Тут вклеена записка. В ней сообщается, что ты приедешь на смену существенно позже. Просят к тебе не придираться сильно."
                                th "Это всё объясняет некоторые странности..."
                            ...
                            "Извиниться":
                                ...
                                th "И сейчас мне больше всего хочется поскорее убраться отсюда и забыть о попытке бесцеремонного вторжения в чужую личную жизнь."
                                jump ds_day2_sl_return
            "Проверить обстановку" (skill='half_light', level=lvl_medium):
                if ds_last_skillcheck.result():
                    window show
                    play sound ds_sfx_fys
                    hfl "{result}Тебе начинает казаться, что кто-то смотрит за тобой."
                    ...
                    window show
                    play sound ds_sfx_fys
                    hfl "{result}Скорее всего, достаточно будет просто посмотреть за дверь."
...
label ds_day2_cards_alone_correct:
    ...
    window hide
    menu:
        "Пометить карты" (skill='interfacing', level=lvl_legendary):
            if ds_last_skillcheck.result():
                window show
                inf "{result}Ты одалживаешь у Слави немного мела, лежащего тут же и, используя его, помечаешь каждую карту."
                $ ds_cards_labeled = True
                
            else:
                window show
                inf "{result}Ты берёшь со стола мел и пытаешься пометить карты..."
...
label ds_day2_tour:
    ...
    if ds_skill_list['half_light'].check(lvl_medium, passive=True).result():
    ...
    menu:
        ...
        "Отказаться" (skill='suggestion', level=lvl_formidable):
            if ds_last_skillcheck.result():
                window show
                play sound ds_sfx_psy
                sug "{result}И пусть всё-таки пойдёт Электроник, ты ведь даже не представляешь, где может быть Женя!"
...
label ds_day2_find_mz:
    ...
    window hide
    menu:
        "Уговорить" (skill='suggestion', level=lvl_challenging):
            if ds_last_skillcheck.result():
                window show
                
                play sound ds_sfx_psy
                sug "{result}Начни убеждать её с того, что тебе эта идея тоже не нравится."
                ...
                play sound ds_sfx_psy
                sug "{result}Но ты даже не знаешь, с чего начать убеждать её."
                ...
                window hide
                menu:
                    "Задержать" (skill='physical_instrument', level=lvl_medium):
                        if ds_last_skillcheck.result():
                            window show
                            play sound ds_sfx_fys
                            phi "{result}Ты хватаешь её так крепко, что она вынуждена остановится."
                            ...
                            window show
                            play sound ds_sfx_fys
                            phi "{result}Но ей удаётся вырвать свою руку из твоей и убежать."
                    ...
                    "Отпустить":
                        ...
                        $ ds_bring_mz_fail = True
                        jump ds_day2_game
        "Заставить" (skill='authority', level=lvl_heroic):
            if ds_last_skillcheck.result():
                
                window show
                play sound ds_sfx_psy
                aut "{result}Пришло время показать ей, что она {i}обязана{/i} пойти на турнир."
                ...
                window show
                play sound ds_sfx_psy
                aut "{result}Ну, попробвать можно... но, скорее, ты её отпугнёшь окончательно."
...
label ds_day2_game:
    if ds_bring_mz_fail:
        ...
        show yn normal pioneer at left with dissolve
        $ ds_met['yn'] = 2
        yn "Вы меня звали, Ольга Дмитриевна?"
        mt "Да. Может, сыграешь в карты? Нам одного человека не хватает."
        show yn surprise pioneer at left with dspr
        yn "Хорошо..."
        play sound ds_sfx_int
        con "И это девушка тоже красива. Может, отметишь это?"
        window hide
        menu:
            "Сделать комплимент" (skill='suggestion', level=lvl_medium):
                if ds_last_skillcheck.result():
                    window show
                    sug "{result}Что может быть проще? Отметь, что у неё волосы красивы, зелёные как трава."
                    ...
                    show yn shy pioneer at left with dspr
                    yn "Да? Спасибо..."
                    $ ds_lp['yn'] += 1
                else:
                    window show
                    sug "{result}Да просто скажи, что она красива."
                    me "Прекрасно выглядишь!"
                    show yn shy2 pioneer at left with dspr
                    yn "Э... хорошо."
                    play sound ds_sfx_psy
    ...
    with dissolve
    me "Давайте начинать."
    if ds_skill_list['suggestion'].check(lvl_easy, passive=True).result():
        play sound ds_sfx_psy
        sug "{result}И обращаешься к Ульяне."
...
label ds_day2_after_tour:
    ...
    $ set_zone_ds_small("house_me_mt", "ds_day2_home")
    $ set_zone_ds_small("forest", "ds_day2_forest")
    $ show_small_map_ds()
...
label ds_day2_medic:
    ...
    window hide
    menu:
        "Продолжать" (skill='instinct', level=lvl_medium):
            if ds_last_skillcheck.result():
                play sound ds_sfx_fys
                ins "{result}Она явно намекает тебе на то, что хочет тебя."
                ins "Более того, ты тоже - во всяком случае, внизу у тебя радости полные штаны."
                me "Да... давайте..."
                
            else:
                play sound ds_sfx_fys
                ins "{result}Ты не понимаешь, к чему она произнесла «лечение» с такой интонацией."
        ...
        "Остановить":
            ...
            jump ds_day2_night
    $ ds_after_tour_who = 'cs'
    scene cg ds_day2_cs_sex1 with dissolve
    "Виола быстро скидывает с себя халат... и под ним не оказывается ничего."
    play sound ds_sfx_fys
    ins "Она смотрит на тебя таким заигрывающим взглядом... она точно {i}хочет{/i} тебя. Как и ты хочешь её."
    play sound ds_sfx_fys
    edr "Вид голого тела Виолы и предвкушение полового акта вызывает у тебя гормональный шторм. Он оказывается настолько сильным, что у тебя расплывается картинка в глазах."
    "Ты расстёгиваешь ширинку своих штанов, обнажая свой член."
    scene cg ds_day2_cs_sex2 with dissolve
    "Ты подносишь его к промежности Виолы, уже склонившейся в удобной позе."
    play sound ds_sfx_int
    enc "{i}Doggy-style{/i}, если быть точным."
    ins "Три... два... один... стыкуйся."
    if ds_skill_list['coordination'].check(lvl_challenging, passive=True, modifiers=[('ds_semtype >= 3', 2, 'Опытен в сексе (не в любви)')]).result():
        play sound ds_sfx_mot
        cor "{result}Успех! Ты попадаешь точно в нужное отверстие!"
        scene cg ds_day2_cs_sex3 with dissolve
        "То, что ты вошёл, моментально отражается в глазах Виолы."
    else:
        play sound ds_sfx_mot
        cor "{result}Ты попал... но не в то отверстие."
        me "Ой!"
        cs "Ой, не туда... а впрочем, продолжай."
        scene cg ds_day2_cs_sex3 with dissolve
    "Ты начинаешь двигаться внутри Виолы. И тебя, и, видимо, её, переполняют приятные ощущения."
    play sound ds_sfx_fys
    ins "Тебе настолько хорошо от того, что случилось, что ты буквально через минуту, не больше, кончаешь."
    play sound ds_sfx_psy
    vol "Поздравляю с детьми!"
    cs "Ах... как хорошо, что я принимаю таблетки..."
    play sound ds_sfx_int
    lgc "Этот Советский Союз всё больше оказывается «альтернативным»... Оральные контрацептивы?"
    play sound ds_sfx_int
    enc "Ничего удивительного, на самом деле. Оральная контрацепция существовала и в «нашем» СССР. Только была в дефиците. Но тут у нас медсестра, по определению имеющая больше доступа к всяким препаратам."
    scene bg int_aidpost_night
    with dissolve
    show cs shy naked at center with dissolve
    ...
    menu:
        "Уйти":
            ...
            "И ты выходишь."
            jump ds_day2_night
        "Заявить протест" (skill='authority', level=lvl_legendary):
            if ds_last_skillcheck.result():
                play sound ds_sfx_psy
                aut "{result}Нет, ты никуда не пойдёшь! Это чёрная неблагодарность!"
                ...
                play sound ds_sfx_psy
                aut "{result}Она выглядит угрожающе, лучше подчинись."
                me "Ну ладно, до свидания..."
                scene bg ext_aidpost_night
                with dissolve
                "И ты выходишь."
            
        "Подловить на словах" (skill='rhetoric', level=lvl_formidable):
            if ds_last_skillcheck.result():
                play sound ds_sfx_int
                rhe "{result}Она пытается от тебя что-то скрыть. Начни выпытывать!"
                ...
                play sound ds_sfx_int
                rhe "{result}Ты не видишь зацепок, она просто хочет, чтобы ты ушёл."
                me "Ну ладно, до свидания..."
                scene bg ext_aidpost_night
                with dissolve
                "И ты выходишь."
    ...
    if ds_skill_list['drama'].check(lvl_challenging, passive=True).result():
        play sound ds_sfx_int
        dra "{result}У госпожи Виолы совершенно точно есть скелеты в шкафу, мессир."
...
label ds_day2_square:
    ...
    window hide
    menu:
        "Поговорить с Гендой" (skill='inland_empire', level=lvl_heroic):
            window show
            ...
            if ds_last_skillcheck.result():
                ...
                window show
                ine "{result}Нет, ты не можешь заставить памятник заговорить."
...
label ds_day2_dock_eve:
    ...
    if ds_skill_list['conceptualization'].check(lvl_medium, passive=True).result():
        play sound ds_sfx_int
        con "{result}Солнце еще не полностью скрылось за горизонтом, и река вдалеке красиво окрашивается во все оттенки красного, желтого и оранжевого."
    ...
    window hide
    menu:
        "Поискать" (skill='perception', level=lvl_challenging):
            window show
            play sound ds_sfx_mot
            ...
            if ds_last_skillcheck.result():
                window show
                per_eye "{result}И вот в одном из последних лучей солнца что-то блестит!"
                ...
                window show
                per_eye "{result}Но ты ничего разглядеть не можешь: совсем стемнело."
...
label ds_day2_house12:
        ...
        "Зайти без стука":
            ...
            menu:
                "Постучать":
                    ...
                    play sound sfx_knocking_door_2
                    "Ты стучишь в дверь."
                "Вломиться в домик" (skill='physical_instrument', level=lvl_challenging):
                    if ds_last_skillcheck.result():
                        play sound sfx_break_cupboard
                        $ renpy.pause(0.5)
                        window show
                        play sound ds_sfx_fys
                        phi "{result}Дверь тут не сильно прочная, так что ты с лёгкостью её выламываешь и попадаешь в домик."
                        window hide
                        ...
                        window show
                        play sound ds_sfx_fys
                        phi "{result}Ты толкаешь дверь, но она не поддаётся."
    ...
    if ds_sl_door_broken:
        ...
        window hide
        menu:
            "Начать домогаться" (skill='instinct', level=lvl_trivial):
                if ds_last_skillcheck.result():
                    window show
                    ins "{result}Вперёд, на покорение этой цели!"
                    ...
                    window show
                    ins "{result}На таком идеальном моменте ты умудрился перенервничать!"
        ...
        "Искал Мику":
            ...
            menu:
                "Согласиться":
                    me "Ладно, вы правы..."
                    jump ds_day2_sleep
                "Настоять на походе сегодня" (skill='suggestion', level=lvl_formidable, modifiers=[('ds_karma <= -50', -2, 'Вожатая не доверяет')]):
                    if skillcheck('suggestion', lvl_formidable):
                        window show
                        play sound ds_sfx_psy
                        sug "{result}Вспомни о столь любимых ею обязанностях пионера."
                        ...
                        window show
                        play sound ds_sfx_psy
                        sug "{result}Но тебе надо пойти!"
                        me "Извините, но мне правда нужно идти!"
                        
                        if ds_karma >= 50:
                            mt "Ну ладно... только обязательно возвращайся не позднее полуночи!"
                            scene bg ext_house_of_mt_night
                            with dissolve
                            "Ты выходишь из домика."
                            jump ds_day2_find_mi
...
label ds_day2_house23:
    ...
    if ds_skill_list['half_light'].check(lvl_trivial, passive=True).result():
        play sound ds_sfx_fys
        hfl "{result}Здесь мало шансов найти Мику и много - неприятностей, кажется."
    $ ds_visited_houses.append(23)
    window hide
    menu:
        "Открыть дверь":
            window show
        ...
        "Уйти" if ds_last_skillcheck.result():
            window show
            th "И то верно."
    ...
    menu:
        ...
        "Подарить заколку":
            ...
            window hide
            menu:
                "Настоять" (skill='suggestion', level=lvl_formidable):
                    if ds_last_skillcheck.result():
                        window show
                        play sound ds_sfx_psy
                        sug "{result}Нет, это подарок, который она примет."
                        ...
                        window show
                        sug "{result}Нет, она не примет у тебя её, она ясно дала понять..."
                        me "Ну ладно..."
                        show dv normal swim at center with dspr
                        dv "Cпокойной ночи!"
                ...
                "Оставить":
                    ...
                    show dv normal swim at center with dspr
                    dv "Cпокойной ночи!"
        "Потребовать быть вежливее" (skill='authority', level=lvl_up_medium):
            if ds_last_skillcheck.result():
                window show
                aut "{result}Она ведёт себя, мягко говоря, агрессивно."
                ...
                window show
                aut "{result}Ну, ты её и правда разбудил..."
                me "Зачем так агрессивно?"
                show dv angry swim at center with dspr
                dv "А чтобы не будил посреди ночи!"
                dv "Короче, ищи помощи в другом месте! Пока!"
            
        "Начать домогаться" (skill='instinct', level=lvl_medium):
            if ds_last_skillcheck.result():
                play sound ds_sfx_fys
                ins "{result}Ты её хочешь! И наверняка это взаимно! Покажи ей!"
                ...
                play sound ds_sfx_fys
                ins "{result}Однако, ты слишком её боишься, чтобы включать свой хорни-режим."
                $ ds_morale -= 1
        ...
        "Вежливо уйти":
            ...
            me "Я, пожалуй, пойду..."
            dv "Вали, пока жив!"
        "Быстро свалить" (skill='savoir_faire', level=lvl_medium):
            if ds_last_skillcheck.result():
                window show
                scene bg ds_ext_houses_night:
                    zoom 1.05 anchor (48,27)
                ...
                with dissolve
                play sound ds_sfx_mot
                svf "{result}У тебя получается легко и быстро убежать."
                ...
                window show
                play sound ds_sfx_mot
                svf "{result}Ты пытаешься убежать, но Алиса останавливает тебя."
                ...
                menu:
                    "Извиниться":
                        ...
                        dv "Ладно, иди уже!"
                    "Обозвать" (skill='reaction_speed', level=lvl_godly):
                        window show
                        play sound ds_sfx_psy
                        ...
                        if ds_last_skillcheck.result():
                            window show
                            play sound ds_sfx_mot
                            res "{result}У тебя получается поставить блок как раз вовремя."
                            ...
                            window show
                            play sound ds_sfx_mot
                            res "{result}Ты не успеваешь отреагировать на её удар."
                            play sound sfx_lena_hits_alisa
                            with vpunch
                            $ ds_health -= 1
                            play sound ds_sfx_fys
                    ...
                    "Осудить" (skill='authority', level=lvl_up_medium):
                        if ds_last_skillcheck.result():
                            window show
                            aut "{result}Она тоже ведёт себя, мягко говоря, агрессивно."
                            me "А ты чего сразу наезжаешь? Может, мне помощь нужна!"
                            show dv angry swim at center with dspr
                            dv "Ладно, но всё равно зачем будить-то?!"
                        else:
...
label ds_day2_house13:
    ...
    menu:
        ...
        "Подарить заколку":
            ...
            jump ds_day2_night
        "Начать домогаться" (skill='instinct', level=lvl_easy):
            if ds_last_skillcheck.result():
                window show
                ins "{result}Ну что ж, давай, покажи ей, как ты её хочешь."
                ...
                window show
                ins "{result}Однако, тебе слишком неудобно приставать к ней."
                play sound ds_sfx_psy
    ...
    menu:
        ...
        "Начать домогаться" (skill='instinct', level=lvl_easy):
            if ds_last_skillcheck.result():
                ...
                play sound ds_sfx_fys
                ins "{result}Ну, покажи ей, что ты её хочешь."
...
label ds_day2_beach:
    ...
    if ds_skill_list['savoir_faire'].check(lvl_legendary, passive=True).result():
        svf "{result}Ты начинаешь захлёбываться, но усилием воли берёшь себя в руки и удерживаешься на воде, схватившись за буек."
    else:
        svf "{result}Ты начинаешь тонуть, но тут кто-то тебя подхватывает за плечи и закидывает на буёк."
        $ ds_dv_rescued = True
    window hide
    ...
    menu:
        ...
        "Начать приставать" (skill='instinct', level=lvl_trivial):
            if ds_last_skillcheck.result():
                window show
                ins "{result}Ты моментально ощущаешь реакцию своего тела от её вида. Вперёд, бери её!"
                ...
                window show
                ins "{result}Однако, ты слишком её боишься..."
                "Ты молчишь."
    ...
    if ds_skill_list['reaction_speed'].check(lvl_heroic, passive=True).result():
        window show
        res "{result}Она и твою одежду забрала!"
    else:
        window show
        play sound ds_sfx_psy
    ...
    window hide
    menu:
        "Задуматься об её поведении" (skill='empathy', level=lvl_challenging, modifiers=[('ds_support_dv > 0', ds_support_dv, 'Поддерживал Алису')]):
            window show
            th "Что это было?"
            ...
            if skillcheck('empathy', lvl_challenging):
                window show
                emp "{result}Ты так и не понял? Она таким образом привлекает твоё внимание."
                ...
                window show
                emp "{result}Да, скорее всего в ней проснулась совесть."
                window hide
                menu:
                    "Начать давить":
                        window show
...
label ds_day2_entrance:
    ...
    window hide
    menu:
        "Последовать за ней" (skill='savoir_faire', level=lvl_formidable):
            if ds_last_skillcheck.result():
                window show
                svf "{result}Тебе удаётся следовать за ней незамеченным вплоть до площади."
                ...
                window show
                play sound ds_sfx_mot
                svf "{result}Тебе под ноги предательски попадает коряга, и ты падаешь."
                ...
                menu:
                    "Начать домогаться" (skill='instinct', level=lvl_easy):
                        if ds_last_skillcheck.result():
                            window show
                            ins "{result}Давай: она явно говорит своим телом, что хочет тебя!"
                            ...
                            window show
                            ins "{result}Однако, даже тут ты слишком боишься и нервничаешь, чтобы показать своё желание."
                    "Спросить про неё":
                        window show
                    "Откланяться и уйти":
                        window show
        ...
        "Выждать":
            window show
            "Ты решаешь не следовать за Славей, а ждёшь, пока она точно не исчезнет."
            window hide
            $ renpy.pause(3.0)
            window show
            th "Всё, думаю, можно идти."
            window hide
            $ renpy.pause(2.0)
            window show
            $ persistent.sprite_time = "night"
            scene bg ext_square_night 
            with dissolve
            "Ты выходишь на площадь и идёшь к себе."
    jump ds_day2_night
...
label ds_day2_badminton:
    ...
    window hide
    menu:
        "Подойти" (skill='volition', level=lvl_challenging, modifiers=[('ds_un_upset', -2, 'Расстроил Лену')]):
            if skillcheck('volition', lvl_challenging):
                window show
                vol "{result}Подойди к ней."
                
            else:
                window show
                vol "{result}Лучше не стоит ей мешать..."
    ...
    menu:
        ...
        "Посмеяться":
            ...
            th "Пойду к себе тогда..."
            jump ds_day2_night
        "Предложить научить" (skill='coordination', level=lvl_legendary):
            window show
    ...
    if ds_last_skillcheck.result():
        ...
        cor "{result}Ты бьёшь по воланчику точно как надо."
        ...
        show un surprise sport at center   with dspr
        cor "{result}Ты бьёшь с такой силой, что он перелетает ограду и скрывается где-то между деревьями."
        me "Ой, прости!"
        ...
        menu:
            "Отправить Лену":
                ...
                "И ты уходишь к себе."
            "}Пойти одному" (skill='perception', level=lvl_heroic):
                window show
                ...
                if ds_last_skillcheck.result():
                    play sound ds_sfx_mot
                    per_eye "{result}Ты осматриваешь каждое дерево в поисках воланчика..."
            ...
            "Пойти с Леной":
                ...
                if ds_skill_list['empathy'].check(lvl_easy, passive=True).result():
                    emp "{result}Тебя обуяла нежность."
                ...
                window hide
                menu:
                    "Успокоить" if ds_last_skillcheck.result():
                        window show
...
label ds_day2_scene:
    ...
    window hide
    menu:
        "Сыграть" (skill='inland_empire', level=lvl_medium):
            if ds_last_skillcheck.result():
                window show
                ine "{result}Ты начинаешь двигать руками, будто держишь гитару."
            else:
                window show
                ine "{result}Твои движения неуклюжи, у тебя не получается нормально представить игру на гитаре."
    ...
    if ds_skill_list['half_light'].check(lvl_medium, passive=True).result():
        window show
        hfl "{result}Она хочет спрыгнуть! Беги за ней!"
        window hide
        menu:
            "Бежать":
            ...
            "Подойти спокойно":
                window show
                "Ты медленно направляешься в ее сторону."
    ...
    window hide
    menu:
        "Ловить" (skill='reaction_speed', level=lvl_up_medium):
            if ds_last_skillcheck.result():
                window show
                play sound ds_sfx_mot
                res "{result}Не задумываясь, ты бросаешься за ней."
                ...
                window show
                play sound ds_sfx_mot
                res "{result}Как ты её поймаешь? А надо ли вообще ловить? А что, если она разобьется? А что, если она тебе что-то сломает? Да и почему именно ты?!"
...
label ds_day2_forest:
    ...
    if ds_skill_list['half_light'].check(lvl_easy, passive=True).result():
        play sound ds_sfx_fys
        hfl "{result}А стоит ли заходить вглубь? Заблудишься ещё."
        window hide
        menu:
            "Пойти дальше":
                pass
    ...
    $ renpy.pause(0.5)
    window show
    show yn surprise pioneer at center with dissolve
    $ ds_met['yn'] = 1
    ...
    if ds_met['yn'] == 2:
        ...
        ynp "Яна... Имерай Яна..."
        $ ds_met['yn'] = 2
    $ ds_after_tour_who = 'yn'
    show yn shy pioneer at center with dspr
    yn "А тебя как зовут?"
    me "Семён."
    yn "Приятно познакомиться..."
    window hide
    menu:
        "Продолжить разговор":
            pass
        "Уйти":
            window show
            me "Ладно, я пойду..."
            show yn normal pioneer at center with dspr
            yn "Пока..."
            "И ты уходишь к себе."
            jump ds_day2_night
    if ds_skill_list['empathy'].check(lvl_easy, passive=True).result():
        ...
        show yn guilty pioneer at center with dspr
        yn "Да нет, я справляюсь... мне Ольга Дмитриевна помогает..."
        me "Это хорошо, умница!"
        show yn surprise pioneer at center with dspr
        yn "Cпасибо..."
        $ ds_lp['yn'] += 1
        show yn smile pioneer at center with dspr
    else:
        window show
        play sound ds_sfx_psy
        ...
        show yn guilty pioneer at center with dspr
        yn "Трудно... у меня же отряд, шестой, дети..."
        play sound ds_sfx_int
        lgc "Она, получается, вожатая, не пионерка."
        play sound ds_sfx_int
        vic "Такая молодая - и уже вожатая? Она явно не старше Слави или Алисы."
...
label ds_day2_library:
    ...
    if not ds_skill_list['shivers'].check(lvl_medium, passive=True).result():
        "Но тут никого нет."
        th "И что мне тут делать?"
        $ disable_current_zone_ds_small()
        jump ds_day2_after_tour
    play sound ds_sfx_fys
    shi "{result}В библиотеке кто-то есть. Сидит и думает о чём-то. Зайди туда!"
    window hide
    menu:
        "Зайти":
            window show
    ...
    $ renpy.pause(0.5)
    menu:
        "Продолжить искать" (skill='perception', level=lvl_medium, modifiers=[('ds_lamp_illuminate', 2, 'Включён свет')]):
            if skillcheck('perception', lvl_medium):
                ...
                per_eye "{result}Ты всё ещё не можешь никого разглядеть. Нет тут никого!"
                th "Ну и ладно!"
                "И ты уходишь к себе."
                jump ds_day2_night
        "Не продолжать":
    ...
    menu:
        "Cпросить про неё":
            ...
            if ds_skill_list['empathy'].check(lvl_easy, passive=True).result():
                emp "{result}Похоже, случай тут непростой... Её что-то в тебе пугает..."
        ...
        "Cпросить про информацию":
            ...
            if ds_skill_list['encyclopedia'].check(lvl_easy, passive=True).result():
                play sound ds_sfx_int
                enc "{result}Общая теория относительности - физическая теория, созданная Альбертом Эйнштейном и помимо прочего описывающая пространство-время как единую сущность."
            ...
            if ds_skill_list['reaction_speed'].check(lvl_medium, passive=True).result():
                play sound ds_sfx_mot
                res "{result}При упоминании Электроника она ненадолго погрустнела."
    ...
    if ds_skill_list['perception'].check(lvl_challenging, passive=True, modifiers=[('ds_lamp_illuminated', 2, 'Включён свет')]).result():
        play sound ds_sfx_mot
        per_eye "{result}Твоё внимание привлекает комок бумаги около стола."
        window hide
        menu:
            "Изучить":
                window show
                ...
                if ds_skill_list['logic'].check(lvl_trivial, passive=True).result():
                    play sound ds_sfx_int
                    lgc "{result}Ж - значит, Женя. А Серёжа - это Электроник."
                    play sound ds_sfx_psy
...
label ds_day2_home:
    ...
    if ds_skill_list['conceptualization'].check(lvl_easy, passive=True).result():
        play sound ds_sfx_int
        con "{result}Тебе в голову приходит интересная мысль - а что, если сыграть в карты с вожатой?" 
        window hide
        menu:
            "Предложить":
                window show
...
label ds_day2_dream:
    ...
    menu:
        ...
        "Уйти":
            ...
            vol "Прости. Я не могу тебе помочь. Тебе придётся поговорить с ней."
    window show
    ...
    if ds_mi_hairpin:
        play sound ds_sfx_mot
        res "У тебя есть заколка. Попробуй сделать ей подарок!"
        window hide
        menu:
            "Подарить":
                window show
                "Ты достаёшь из кармана заколку и вручаешь ей."
                show dvw normal at center with dspr
                dv "Нет, я не возьму."
                res "Как?!"
                me "Но... почему?"
                dv "Я не хочу подарков. Я хочу уехать."
                emp "Она считает, что, приняв подарок, она должна будет давать тебе в ответ. А она не хочет ничего тебе давать."
                res "Но это... должно было сработать."
                vol "Как способ коммуникации, когда слова не работают?"
                res "Да, именно!"
                vol "Это могло сработать раньше. Но не теперь. Эта идея была наивной."
            "Не дарить":
                window show
    play sound ds_sfx_int
    ...
    menu:
        ...
        "Возмутиться":
            window show
            me "То есть, ты серьёзно меня бросаешь одного, без какого-либо уважения, зато за каких-то друзей переживаешь!"
            show dvw smile at center with dspr
            dv "Было бы странно, будь бы иначе. Ты - прошлое для меня. А друзья - будущее."
            dv "Я стремлюсь к будущему. И тебе было бы неплохо."
            aut "Как... Не может быть... Она тебя не послушала..."
            aut "Кажется, я вне игры. Дальше без меня..."
        "Промолчать":
            window show
    play sound ds_sfx_fys
    ...
    window hide
    menu:
        "Поцеловать" (skill='suggestion', level=lvl_medium):
            $ skillcheck('suggestion', lvl_medium)
            window show
            show dvw normal:
                xalign 0.5 yalign 0.5
                linear 1.0 zoom 1.2
            sug "{result}Трясущимися ногами ты пододвигаешься ближе к ней. Её тело приближается к тебе. Такое тёплое... Закрыв глаза, ты тянешься своим ртом к её губам."
    ...
    menu:
        "Продолжать":
            ...
            me "Увидимся ли мы снова?"
            dv "Я тебя - нет, а вот ты меня - обязательно!"
            me "Это как?"
            dv "Да как ты не понимаешь?! Это сон! Я уже давным-давно уехала. От тебя!"
            me "А как же я тебя увижу?"
            dv "Когда ты начинаешь думать об этом - сны повторяются снова и снова... не менее трёх раз в неделю."
            ine "Она права - этот сон тебя преследует постоянно."
            me "Это ужасно..."
            show dvw normal at center with dspr
            dv "И то верно... Это истинная тьма. Настоящее зло - это не войны, не убийства или изнасилования. Настоящее зло предстаёт в облике любви."
            dv "Надеюсь, ты поймёшь это, Семён. Когда-нибудь..."
        "Попрощаться":
            window show
            me "Прощай..."
    show dvw normal at center with dspr
