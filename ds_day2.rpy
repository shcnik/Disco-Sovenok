...
label ds_day2_pass_alone_music:
    ...
    menu:
        "Вспомнить":
            if skillcheck('encyclopedia', lvl_medium):
                window show
...
label ds_day2_pass_sl_music:
    ...
    $ ds_met['mi'] = 1
    play sound ds_sfx_fys
    ins "Она стоит на четвереньках в такой пикантной позе, что ты не сразу начинаешь разговор..."
...
label ds_day2_pass_un_music:
    ...
    "Ты наклоняешься и видишь девочку, которая, кажется, что-то искала."
    play sound ds_sfx_fys
    ins "Она стоит на четвереньках в такой пикантной позе... не будь бы Лены, ты бы начал приставать."
    me "Простите..."
    window hide
    ...
    menu:
        "Спросить про Алису":
            ...
            if skillcheck('drama', lvl_easy):
                ...
                window show
                play sound ds_sfx_int
                dra "Но она, кажется, не врёт и точно ничего не скрывает."
            play sound ds_sfx_psy
...
label ds_day2_cards_alone:
    ...
    if skillcheck('shivers', lvl_medium):
        play sound ds_sfx_fys
    ...
    if ds_sl_keys:
        ...
        menu:
            "Использовать ключи":
                ...
                "Ты подскакиваешь на месте, резко разворачиваешься, но в домике никого, кроме тебя, нет."
                th "Наверное, ветер..."
                window hide
                menu:
                    "Проверить":
                    ...
                    "Продолжить изучать":
                        ...
                        per_eye "ВНИМАНИЕ! Обратите особое внимание, имеются проблемы в поведении, обусловленные... (дальше неразборчиво)"
                        play sound ds_sfx_psy
                        emp "Оно и заметно."
                        emp "Жаль, причины не видно, придётся выяснять самим."
                        ...
                        "Ты закрываешь ящик и выходишь из домика вожатой."
        window hide
...
label ds_day2_beach:
    ...
    window show
    "Ты оборачиваешься и видишь Алису, плывущую за мной."
    me "Ты что делаешь?!"
    ...
    if skillcheck('reaction_speed', lvl_heroic):
        window show
...
label ds_day2_entrance:
    ...
    enc "Эта ситуация больше всего напоминает пантеизм – растворение Бога в природе, во всем сущем."
    play sound ds_sfx_psy
...
label ds_day2_scene:
    ...
    menu:
        ...
        "Не ловить":
            ...
            play sound ds_sfx_fys
            edr "Как ты её поймаешь? А надо ли вообще ловить? А что, если она разобьется? А что, если она тебе что-то сломает? Да и почему именно ты?!"
...
label ds_day2_sleep:
    ...
    vol "Ты хочешь лишь, чтобы от меня все отстали – в первую очередь твои мысли.{w} Хочешь, чтобы все разрешилось как-нибудь само собой."
    vol "Ну, или по крайней мере без твоего деятельного участия."
    play sound ds_sfx_psy
    ine "А вдруг ты тут навсегда?"
    ine "Тогда придется привыкать..."
    ine "Но как вот так просто... Я... Я не готов...{w} Эх..."
...
label ds_day2_dream:
    ...
    dv "Всё прошло, мне нужно уезжать. А ты останешься один - навеки."
    play sound ds_sfx_mot
    ...
    menu:
        ...
        "Бывшая кто-то там":
            ...
            th "Все?"
            arb "Да, как бы все."
            lim "{i}Буквально{/i} все. От глубин подсознания до последнего нейрона неокортекса."
